...
# -----------------------------
# Wrapper for router.py
# -----------------------------
def ask(query: str):
    # keyword search
    keyword_answer = answer_query_keyword(query)
    if keyword_answer:
        return {"query": query, "answer": keyword_answer}

    if llama_agent is None:
        return {"query": query, "answer": "⚠ LLaMA agent not available. Only keyword-based search is supported."}

    try:
        response = llama_agent.run(query)
        return {"query": query, "answer": f"🦙 LLaMA says: {response.strip()}"}
    except Exception as e:
        return {"query": query, "answer": f"⚠ LLaMA query failed: {e}"}

# -----------------------------
# APIRouter for mounting
# -----------------------------

backend1_router = APIRouter()
...
@backend1_router.post("/ask")
def ask_endpoint(request: QueryRequest):
    return ask(request)   # make sure "ask" is imported from wherever you defined it
